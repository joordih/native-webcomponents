import { usersTableConfig, usersFormConfig } from "./users-config";
...
class UsersComponent extends HTMLElement {
    ...
    this.shadow.innerHTML = `
      ...
      <div class="users-content">
        ...
        <generic-forms 
          ...
          mode="${this.formMode}"
          ${this.selectedUser ? `data='${JSON.stringify(this.selectedUser)}'` : ""}
        ></generic-forms>
      </div>
    `;

    // Set config objects directly after rendering
    const datatable = this.shadow.querySelector("generic-datatable") as any;
    const forms = this.shadow.querySelector("generic-forms") as any;

    if (datatable) {
      datatable.setConfig(usersTableConfig);
    }

    if (forms) {
      forms.setConfig(usersFormConfig);
    }
  }
  ...
  private showEditForm(user: any): void {
    this.currentView = "form";
    this.formMode = "edit";
  ...
  private async handleDelete(user: any): Promise<void> {
    ...
    try {
      const response = await fetch(
        `${usersTableConfig.apiEndpoint}/${user.id}`,
        {
          method: "DELETE",
        }
      );
